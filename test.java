...
public class test {
    ...
    private static final int[][] ALL_SEQUENCES = {
        ...
        {1, 3, 5, 6, 8, 11, 12, 15},
        {60, 62, 64, 65, 66, 68, 70, 71},
        {12, 9, 7, 5, 2},
        {32, 29, 27, 24, 22, 19, 16, 14},
        {56, 53, 51, 50, 49, 47, 44},
    ...
    private static boolean isProgressive(int[] numbers) {
        ...
        for (int i = 0; i < numbers.length - 1; i++) {
            ...
            // Condition de Progression: 1 <= diff <= 3
            if (diff < 1 || diff > 3) {
                return false;
            }
        }
    ...
    private static boolean isRegressive(int[] numbers) {
        if (numbers.length <= 1) return true;

        for (int i = 0; i < numbers.length - 1; i++) {
            int diff = numbers[i] - numbers[i + 1];
            // Condition de Régression: 1 <= diff <= 3 (le nombre précédent est plus grand)
            if (diff < 1 || diff > 3) {
                return false;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        int totalLines = ALL_SEQUENCES.length;

        System.out.println("🚀 Début de l'analyse des séquences stockées en int[][].");
        System.out.println("---");

        // Traiter chaque séquence dans le tableau ALL_SEQUENCES
        for (int[] numbers : ALL_SEQUENCES) {
            
            // Appliquer la condition : soit strictement croissante, soit strictement décroissante.
            boolean isValid = isProgressive(numbers) || isRegressive(numbers);
